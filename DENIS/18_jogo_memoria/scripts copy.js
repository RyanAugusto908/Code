...
cartas.forEach(carta => {
  carta.addEventListener('click', () => {
    // Impede clicar de novo durante a animação
    if (carta.classList.contains('flip')) return;

    carta.classList.add('flip');

    // Depois de 2s (duração da animação), a carta fica parada revelada
    setTimeout(() => {
      // aqui ela já está virada — se quiser revelar logicamente no jogo, faça aqui
      // exemplo: carta.classList.add('revelada');
    }, 2000);
  });
});
